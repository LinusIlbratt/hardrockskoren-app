// functions/event/markGeneralUpdateAsSeen.ts
...
export const handler = middy<AuthorizedEvent, APIGatewayProxyResultV2>().handler(
  async (event): Promise<APIGatewayProxyResultV2> => {
    try {
        ...
        const seenUpdatesAttr = UserAttributes?.find(attr => attr.Name === 'custom:seenGenUpd');
        const seenUpdates: Record<string, string> = seenUpdatesAttr?.Value ? JSON.parse(seenUpdatesAttr.Value) : {};
        seenUpdates[eventId] = updatedAt;
        ...
        await cognitoClient.send(new AdminUpdateUserAttributesCommand({
            ...
            UserAttributes: [{ 
                // ÄNDRING: Använder det nya, kortare namnet
                Name: 'custom:seenGenUpd', 
                Value: JSON.stringify(seenUpdates) 
            }],
        }));
        
        return sendResponse({ message: "Successfully marked general update as seen." }, 200);
    } catch (error: any) {
        console.error("Error marking general update as seen:", error);
