...
export const handler = async (
  ...
  try {
    ...
    const linked: { groupSlug: string; repertoireId: string }[] = [];
    const notLinked: string[] = [];

    for (const groupSlug of slugsResult.slugs) {
      const match = await findLinkedRepertoire(MAIN_TABLE, groupSlug, folderPath);
      if (match) {
        linked.push(match);
      } else {
        notLinked.push(groupSlug);
      }
    }
    ...
    console.error("libraryFolderChoirStatus failed:", error);
    const message =
